const {
  ...
  async function startBot() {
    ...
    // === WELCOME / BYE HANDLER ===
    const welcomePlugin = require('../plugins/21_welcome_bye');
    ...
    sock.ev.on('group-participants.update', async ({ id: groupId, participants, action }) => {
      try {
        ...
        for (const participant of participants) {
          ...
          if (action === 'add') {
            const welcomeOn = db.getNote(groupId, '__welcome_on__');
            if (welcomeOn !== '1') continue;

            const styleIdx = parseInt(db.getNote(groupId, '__welcome_style__') || '0');
            const idx = isNaN(styleIdx) ? Math.floor(Math.random() * welcomePlugin.welcomeMessages.length) : styleIdx % welcomePlugin.welcomeMessages.length;
            const text = welcomePlugin.formatMsg(
              welcomePlugin.welcomeMessages[idx],
              '@' + userNum,
              groupName,
              memberCount
            );
            await sock.sendMessage(groupId, { text, mentions: [participant] });
            logger.info(`👋 Welcome envoyé à ${userNum} dans ${groupName}`);

          } else if (action === 'remove' || action === 'leave') {
            const byeOn = db.getNote(groupId, '__bye_on__');
            if (byeOn !== '1') continue;

            const idx = Math.floor(Math.random() * welcomePlugin.goodbyeMessages.length);
            const text = welcomePlugin.formatMsg(
              welcomePlugin.goodbyeMessages[idx],
              userNum,
              groupName,
              memberCount
            );
            await sock.sendMessage(groupId, { text });
            logger.info(`👋 Bye envoyé pour ${userNum} dans ${groupName}`);
          }
        }
      } catch (err) {
        logger.error('Erreur welcome/bye:', err.message);
      }
    });
